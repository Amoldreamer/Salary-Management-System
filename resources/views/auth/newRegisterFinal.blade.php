                        ...
                        <div class="col-xl-8 offset-xl-2 bdiv" style="height:auto;">
                            <nav class="navbar navbar-expand-sm navbar-dark bg-dark">
                                ...
                                <ul class="navbar-nav ml-auto">
                                    ...
                                    <div class="dropdown-menu" style="background-color:rgb(176, 177, 207);" aria-labelledby="mydropdown">
                            ...
                            <table class="table " style="width:850px">
                                    <tr>
                                        ...
                                        <td>Role</td>
                                        <td>Mobile</td>
                                        <td style="margin-left:-60px">Address</td>            
                                    </tr>
                                    @foreach($data as $key=>$value)
                                    <tr>
                                        <td><img src="public/uploads/highlights/{{$value->photo}}" style="height:50px;width:50px;" /></td>
                                        <td>{{$value->name}}</td>
                                        <td>{{$value->assumed_role}}</td>
                                        <td>{{$value->mobile}}</td>
                                        <td>{{$value->address}}</td> 
                                        <td><a href="addSalary?register={{$value->Register}}" ><button type="submit" class="btn btn-primary">
                                            {{ __('Add Salary') }}
                                        </button></a></td> 
                                        <td><a href="editEmployees?register={{$value->Register}}" ><button type="submit" class="btn btn-success">
                                            {{ __('Edit') }}
                                        </button></a></td> 
                                        <td><a onclick="return confirm('Are you sure you want to delete this item?');" href="deleteEmployees?register={{$value->Register}}" ><button type="submit" class="btn btn-danger">
                                            {{ __('Delete') }}
                                        </button></a></td> 
                                    </tr>
                                    @endforeach
